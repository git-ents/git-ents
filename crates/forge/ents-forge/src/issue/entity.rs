...
/// rather than a fixed enum (contrast [`ents_model::Status`], which *is* a
/// fixed taxonomy because `model.result-taxonomy` says so explicitly).
/// Extending this struct with further fields later is a storage migration
/// (`model.extensibility`, `meta-ref.migration`), not a platform request.
///
/// # Examples
///
/// ```
