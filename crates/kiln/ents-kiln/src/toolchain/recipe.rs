...
//! `model.toolchain`).
//!
//! [`super::Toolchain::recipe`] is deliberately an opaque `String` —
...
/// [`Toolchain::recipe`] (`effect.toolchains`).
...
impl Recipe {
    /// Parse a [`Recipe`] out of a [`Toolchain::recipe`] string.
    ///
...
/// use ents_kiln::toolchain::resolve;
...
/// assert!(matches!(recipe, ents_kiln::Recipe::Embedded { .. }));
/// ```
pub fn resolve(
...
/// ```
/// use ents_kiln::Recipe;
/// use ents_kiln::toolchain::materialize;
...
pub fn materialize(recipe: &Recipe, objects: &impl Find, cache_root: &Path) -> Result<PathBuf> {
    ...
    match recipe {
        Recipe::Embedded { tree } => {
            ...
            make_dir(&bin_tmp)?;
            ents_effect::materialize::checkout(objects, *tree, &bin_tmp)?;
        }
