...
pub enum Error {
    ...
    /// A typed-tree entity (a [`ents_model::Status`]) could not be
    /// (de)serialized.
    ...
    /// The named entity (a toolchain, looked up by something other than
    /// its recipe — e.g. `ents-kiln`'s `toolchain::log`) does not exist.
    /// Distinct from [`Error::UnknownToolchain`], which names the
    /// resolve-a-recipe failure specifically; this variant is the general
    /// "nothing at that name" case a caller renders as a not-found error.
    ...
    /// A toolchain's `recipe` field did not parse as a `Recipe`
    /// (`ents-kiln`'s own type; `effect.toolchains`: "a manifest's declared
    /// components MUST be resolved during effect execution").
    ...
    /// A `Component` (`ents-kiln`'s own type) carried a `dest` that is not
    /// empty or a single safe path segment, a `url` that is empty or
    /// carries whitespace or a quote, or a `sha256` that is not 64 hex
    /// characters — all of which feed filesystem paths, `curl`, or an
    /// in-sandbox shell string downstream. Checked when a recipe is parsed
    /// and re-checked before any fetch.
    ...
    /// Writing a new object to the object store failed (building a
    /// toolchain import's tree, `ents-kiln`'s `toolchain::import`).
    #[error(transparent)]
