...
//! extracted bytes survive between runs (`ents-kiln`'s toolchain
//! `materialize`'s host-side cache has a Sprite-side mirror, `sync_dir`'s
//! extract-once check).
//!
...
//! (the same [`crate::materialize::checkout`] and `ents-kiln`'s toolchain
//! `materialize` every backend shares), then `tar`'d
...
/// key `ents-kiln`'s toolchain `materialize` cached `host_bin`'s parent
...
/// The content key `ents-kiln`'s toolchain `materialize` cached `host_bin`
