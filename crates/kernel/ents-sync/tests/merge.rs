...
/// `ents-forge`, which itself depends on `ents-sync`'s sibling kernel
/// crates): a rich, multi-field entity — two scalar strings, a
/// restricted-value string, and two collections — covering every field
/// shape the schema-aware merge must handle field-by-field.
